...
export async function PUT(request: Request, { params }: Context) {
    try {
        ...
        if (data?.user?.role !== "admin") return NextResponse.json({ message: "You are not authorized to perform this action.", success: false, data: {} }, { status })

        if (request.method !== "PUT") return NextResponse.json({ message: "Method is not allowed.", success: false, data: {} }, { status: 405 })
        ...
        const updatedPost = await Post.findByIdAndUpdate(id, { title, excerpt, media: { featuredImage }, content, seo: { metaTitle, metaDescription } }, { new: true, runValidators: false })

        if (!updatedPost) return NextResponse.json({ message: "User not found.", success: false, data: {} }, { status: 404 })
